...
class VanillaRNN(object):
    ...
    def step_backward(self, dnext_h, meta):
        ...
        dactivation = 1 - np.square(next_h) 
        dx = (dnext_h*dactivation)@(self.params[self.wx_name].T) 
        dprev_h = (dnext_h*dactivation)@(self.params[self.wh_name].T)
        dWh = prev_h.T@(dnext_h*dactivation)
        dWx = x.T@(dnext_h*dactivation)
        db = (dnext_h*dactivation).sum(axis=0)
        if (self.grads[self.wx_name] is not None and self.grads[self.wh_name] is not None 
                                                and self.grads[self.b_name] is not None):
    ...
    def forward(self, x, h0):
        ...
        for i in range(x.shape[1]):
            prev_h, meta_i = self.step_forward(x[:,i,:], prev_h)
...
############################################################################
    ...
    def backward(self, dh):
        ...
        for i in range(T - 1, -1, -1):
            dx_i, dprev_h, dWx, dWh, db = self.step_backward(total_gradient, self.meta[i])  

            dx[:,i,:] = dx_i
            
            total_gradient = dprev_h 

            if i != 0: 
                total_gradient += dh[:,i-1,:]
...
class LSTM(object):
    ...
    def step_forward(self, x, prev_h, prev_c):
        ...
        meta=[x, prev_h, prev_c, A_i, A_f, A_o, A_g, np.tanh(next_c)]
        #############################################################################
        #                               END OF YOUR CODE                            #
        #############################################################################
    ...
    def step_backward(self, dnext_h, dnext_c, meta):
        ...
        #############################################################################
        x, prev_h, prev_c, A_i, A_f, A_o, A_g, tanh_next_c=meta
        do = tanh_next_c*dnext_h
        dio = A_o*(1 - A_o)*do
        dnext_c= A_o*(1 - tanh_next_c**2)*dnext_h+dnext_c
        dprev_c = A_f*dnext_c
        df = prev_c*dnext_c
        dif = A_f*(1 - A_f)*df
        di = A_g*dnext_c
    ...
    def backward(self, dh):
        ...
        for i in range(T - 1, -1, -1):
            dx_i, dprev_h, dprev_c, dWx, dWh, db = self.step_backward(total_gradient,dprev_c, self.meta[i])
            dx[:,i,:] = dx_i
            total_gradient = dprev_h
            if i != 0: 
                total_gradient += dh[:,i-1,:]
